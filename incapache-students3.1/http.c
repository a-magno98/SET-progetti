...
/*** TO BE DONE 3.0 START ***/
	...
	if( !(gmtime_r(&now_t,&now_tm)) )
		fail_errno("Unable to convert data");
...
/*** TO BE DONE 3.0 START ***/
			...
			mime_type = (char*)(my_malloc(strlen(HTML_mime)));
...
/*** TO BE DONE 3.0 START ***/
		...
		exp_date.tm_mday = 31;
		exp_date.tm_mon = 11;
		exp_date.tm_year = 119;

		strftime(time_as_string, MAX_TIME_STR, "%a, %d %b %Y %T GMT", &exp_date);
...
/*** TO BE DONE 3.0 START ***/
		...
		off_t offset = 0;
		size_t sent_bytes;
      	while (((sent_bytes = sendfile(client_fd, fd, &offset, file_size)) > 0) && (file_size > 0))
...
/*** TO BE DONE 3.0 START ***/
						...
						option_val = strchr(strtokr_save, '=');
						UIDcookie = atoi(option_val+1);
...
/*** TO BE DONE 3.0 START ***/
					...
					if ( strcmp(option_name, "If-Modified-Since") == 0 )
					{
						...
						strptime(strtokr_save+1, "%a, %d %b %Y %T GMT", &since_tm);
					}
