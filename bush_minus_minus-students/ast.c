...
/*** TO BE DONE START ***/
	...
	if(!impl->pathname)
		new_path = (char* const)vt_lookup(sh_get_var_table(sh), HOME);
	else
		new_path = impl->pathname;

	if(chdir((const char*)new_path) < 0)
		fprintf(stderr, "bush: @cd: %s: %s\n", impl->pathname ,strerror(errno));

	vt_set_value(sh_get_var_table(sh),PWD, new_path);
...
struct node *new_node_cd(char *const pathname)
{
	...
	struct cd_impl *impl;
#ifdef DEBUG
	n->dump_to_stdout = cd_dump_to_stdout; //puntatore a funzione
#endif /* #ifdef DEBUG */
...
/*** TO BE DONE START ***/
	...
	char* tmp_path;
	char* aux_path = (char*)path;
	size_t dim;

	for(;;)
	{
		
		if((tmp_path = strchr(aux_path, DIR_SEPARATOR)))
			dim = strlen(aux_path)-strlen(tmp_path);
		else
			dim = strlen(aux_path);

		char* percorso = my_malloc(sizeof(char)*(dim+strlen(name)+2));
		strncpy(percorso, aux_path, dim);
		percorso[dim] = PATH_SEPARATOR;
		percorso[dim+1] = 0;
		strcat(percorso, name);
...
/*** TO BE DONE START ***/
	if(from_fd != NO_REDIR)
	{
		if(dup2(from_fd, to_fd) < 0)
			fail_errno("Unable to create a copy of the file descriptor");
...
/*** TO BE DONE START ***/
	...
	if((pid = fork()) < 0)
		fail_errno("Unable to fork a new process");
...
/*** TO BE DONE START ***/
		...
		if(execve(executable, impl->args, sh_get_environment(sh)) < 0)
			fail_errno("Unable to execute a command");
...
/*** TO BE DONE START ***/
	 ...
	 if(pipe(pipes) < 0)
		fail_errno("Could not init pipes");
