...
/*** TO BE DONE START ***/
    ...
    char** list=(char**)my_malloc(sizeof(char*)*(this->len+1));

  	for(size_t i=0;i<this->len; i++)
	{
        list[i]=(char*)my_malloc(sizeof(char)*(strlen(this->vars->name)+strlen(this->vars->value)+2));
    	   sprintf(list[i], "%s=%s", this->vars->name, this->vars->value);
    }
...
struct var_table *vt_new()
{
	...
	vt->len = 0; // gli elementi che ci sono dentro
	vt->capacity = 8; // tutti gli elementi che possono stare dentro 
