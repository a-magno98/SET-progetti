...
/*** TO BE DONE START ***/
	...
	if( (sent_bytes = send(tcp_socket, message, msg_size, 0)) < 0)
...
/*** TO BE DONE START ***/
   	...
   	if( (tcp_socket = socket(gai_hints.ai_family, gai_hints.ai_socktype, gai_hints.ai_protocol)) < 0)
		fail_errno("TCP Ping cannot create socket");

	if ( connect (tcp_socket, (struct sockaddr *)ipv4, sizeof(*ipv4) ) != 0)
		fail_errno("TCP Ping cannot connect to server");
...
/*** TO BE DONE START ***/
    ...
    if( write(tcp_socket, request, strlen(request)) < 0)
		fail_errno("TCP Ping cannot write to the server");
...
/*** TO BE DONE START ***/
	...
	if(strncmp(answer,"OK", 2) !=0)
 	   fail("TCP Ping: unexpected answer from server");
