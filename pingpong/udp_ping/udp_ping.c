...
/*** TO BE DONE START ***/
		...
		{
			last_sent = send(ping_socket, message+last_sent, msg_size-sent_bytes, MSG_DONTWAIT);
	
			if(last_sent == -1)
			{
				if(errno != EAGAIN && errno != EWOULDBLOCK)
					fail_errno("Cannot send message to server");
				else
					sent_bytes = 0;
			}
			else
...
/*** TO BE DONE START ***/
	...
	if(fcntl(ping_socket,F_SETFL, O_NONBLOCK) <0)
		fail_errno("set NONBLOCKING socket failed");
...
/*** TO BE DONE START ***/
	...
	if( (ask_socket = socket(gai_hints.ai_family, gai_hints.ai_socktype, gai_hints.ai_protocol)) < 0)
		fail_errno("UDP Ping cannot create socket");

	if ( connect (ask_socket, (struct sockaddr *)ipv4, sizeof(*ipv4) ) != 0)
		fail_errno("UDP Ping cannot connect to server");
...
/** TO BE DONE START ***/
	 ...
	 if( write(ask_socket, request, strlen(request)) < 0)
		fail_errno("UDP Ping cannot write to the server");
...
/*** TO BE DONE START ***/
    ...
    if(strncmp(answer,"OK", 2) !=0)
 	   fail("UDP Ping: unexpected answer from server");
